...
interface TransferAccess {
        ...
        return new TransferAccess() {
            ...
            public @NotNull CompletableFuture<TransferReport> importFrom(@NotNull String pluginName,
                ...
                return Transfers.of(database.plugin()).importFrom(file, force).thenApply(report -> {
                    ...
                    // to again, whether or not a wipe froze it a minute ago.
                    if (report.outcome() != TransferOutcome.FAILED) {
                        database.tables().values().forEach(Repository::thaw);
                    }
                    return report;
                });
            }
            ...
            public @NotNull CompletableFuture<TransferReport> wipe(@NotNull String pluginName,
                ...
                return wiped.thenApply(report -> {
                    if (report.outcome() != TransferOutcome.FAILED) {
                        frozen(database, table).forEach(Repository::freeze);
                    }
                    return report;
                });
            }
    ...
    /**
     * The repositories a wipe of one table, or of all of them, has emptied.
     *
     * <p>They are frozen until the server restarts. Without that a wipe only
     * looked like one: the plugin still held every row in memory — a clan
     * manager indexes its tables on enable and never reads them again — and
     * wrote them back on its next save, which on a server stop is every
     * online player's quit. The admin restarted, the data was there, and the
     * wipe had done nothing visible. Case-insensitive on the name for the same
     * reason the wipe itself is.
     */
    private static List<Repository<?>> frozen(PluginDatabase database, @Nullable String table) {
        return database.tables().entrySet().stream()
                .filter(entry -> table == null || entry.getKey().equalsIgnoreCase(table))
