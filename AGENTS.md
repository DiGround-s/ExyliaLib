...
| util (mob) | `util/mob/Mobs`, `PluginMobs`, `MobTemplate`, `MobSkill`, `MobFlag`, `MobDeath`, `MobCodec`; `MobBehaviour`, `MobLook`, `MobHit` (1.195.0); `MobSkill.Cast`/`Gate`/`Aim`, `MobFight`, `MobPhase`, `PluginMobs.fightEditor` (1.198.0); `MobVisuals`, `PluginMobs.visuals`, `MobLook` reactions (1.199.0) | `util/mob/internal/` (`MobEngine` listeners/spawn/what each type does/hits mode/look, one entity timer per mob — every 2 ticks only for auras, `<rainbow>` names and `WANDERS`; `MobCaster` conditions, rotation groups, staged casts as delayed entity tasks, chains, phases; `MobAim` pure aim geometry; `MobReactions` entrances/hits/heals/deaths/low look, AUTO picks, large-effect cap; `MobBodies` humanoid skins, heads, palettes, vanish; `LiveMob` cooldowns, groups, global cooldown, cast under way, phase, damage ledger, hits, leash); `util/mob/MobSkillDescriptor`, `MobPhaseDescriptor`; released in `ExyliaLib.onPluginDisable` | [docs/mobs.md](docs/mobs.md) | 1.192.0 |
