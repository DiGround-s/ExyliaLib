...
| timed visual effects built in Java (timeline, telegraphs, floating numbers, screen shake, chained motions, flat ragdoll skins) | `display/Vfx`, `VfxRun`, `Telegraphs`, `Indicators`; `DisplayMotion.chain`; `ragdoll/RagdollSkin.flat` | `packet/internal/ScreenShake`; `Steps.Shake` and `[SHAKE]` in `SequenceCompiler` | [docs/vfx.md](docs/vfx.md) | 1.197.0 |
...
| util (showcase) | `util/showcase/Showcases`, `PluginShowcases`, `ShowcaseSettings`, `ShowcaseAct`, `ShowcaseStage`, `ShowcaseTurn` | `util/showcase/internal/LiveStage` | [docs/showcases.md](docs/showcases.md) | 1.188.0 |
| util (crate) | `util/crate/Crates`, `PluginCrates`, `CrateCatalogue`, `CrateSettings`, `CrateMessages`, `CrateTier`, `CrateReward` | `util/crate/internal/` (`CrateStore`, `CrateRow` table `exylia_crate_players`, `Prizes`, `TierTable`, `Reel`, `CrateScreens`, `BoundBlocks`) | [docs/crates.md](docs/crates.md) | 1.189.0 |
| util (mob) | `util/mob/Mobs`, `PluginMobs`, `MobTemplate`, `MobSkill`, `MobFlag`, `MobDeath`, `MobCodec`; `MobBehaviour`, `MobLook`, `MobHit` (1.195.0); `MobSkill.Cast`/`Gate`/`Aim`, `MobFight`, `MobPhase`, `PluginMobs.fightEditor` (1.198.0); `MobVisuals`, `PluginMobs.visuals`, `MobLook` reactions (1.199.0); `MobTheme`, `MobSkills` library and styles, `PluginMobs.theme`/`preview`, `DASH`/`CHAIN`/`SHIELD`/`ZONE`/`BARRAGE` (1.200.0) | `util/mob/internal/` (`MobEngine` listeners/spawn/what each type does/hits mode/look, one entity timer per mob — every 2 ticks only for auras, `<rainbow>` names and `WANDERS`; `MobCaster` conditions, rotation groups, staged casts as delayed entity tasks, chains, phases; `MobAim` pure aim geometry; `MobReactions` entrances/hits/heals/deaths/low look, AUTO picks, large-effect cap; `MobBodies` humanoid skins, heads, palettes, vanish; `MobStyle` style recipes; `MobShows` plays styles, over-time parts, preview; `MobMoves` dash, chain, shield, zone, barrage, hard landing; `Stage` one cast; `Shapes` models, colours, sounds; `LiveMob` cooldowns, groups, global cooldown, cast under way, phase, damage ledger, hits, leash); `util/mob/MobSkillDescriptor`, `MobPhaseDescriptor`; released in `ExyliaLib.onPluginDisable` | [docs/mobs.md](docs/mobs.md) | 1.192.0 |
...
| packet | `packet/Packets`, `PluginPackets`, `Visibility`, `VisibilityRule`, `Reveal` + `RevealStyle` (1.116.0), `FakeBlocks`, `Movement`, `FakeGameMode`, `SilentContainer`, `WorldBorders` + `VirtualBorder` (1.162.0) | `packet/internal/` (`PacketRuntime`, `PacketSink`, `SectionGroups`, `Mirrors`, `Borders` — one border per viewer in `SEEN`, resize kept as a clock, damage on an entity timer; PacketEvents confined to `PacketHooks`) | [docs/packets.md](docs/packets.md) | 1.75.0 |
...
| metrics | `metrics/Metrics` (`describe`, 1.163.0) | `metrics/internal/` (`MetricsRuntime` — inventory and details resent only when changed, `ErrorGroups`, `ServerErrors`); `start` in `ExyliaLib.onEnable`, `stop` in `onDisable`, `release` in `onPluginDisable`; hooks in `AbstractTaskScheduler.report` and `Debug.send`; `metrics` block in `LibrarySettings` | [docs/metrics.md](docs/metrics.md) | 1.160.0 |
...
| cross-server lease locks | `database/RowLocks` | `database/internal/LockRow` (`exylia_row_locks`) | [docs/database.md](docs/database.md) | 1.163.0 (`holdRenewing`, `Lease` 1.167.0) |
