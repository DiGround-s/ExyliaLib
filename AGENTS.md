...
| util (mob) | `util/mob/Mobs`, `PluginMobs`, `MobTemplate`, `MobSkill`, `MobFlag`, `MobDeath`, `MobCodec` | `util/mob/internal/` (`MobEngine` listeners/spawn/skills, one entity timer per mob; `LiveMob` cooldowns and damage ledger); `util/mob/MobSkillDescriptor`; released in `ExyliaLib.onPluginDisable` | [docs/mobs.md](docs/mobs.md) | 1.192.0 |
