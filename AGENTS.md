...
| util (mob) | `util/mob/Mobs`, `PluginMobs`, `MobTemplate`, `MobSkill`, `MobFlag`, `MobDeath`, `MobCodec`; `MobBehaviour`, `MobLook`, `MobHit` (1.195.0) | `util/mob/internal/` (`MobEngine` listeners/spawn/skills/hits mode/look, one entity timer per mob — every 2 ticks only for auras, `<rainbow>` names and `WANDERS`; `LiveMob` cooldowns, damage ledger, hits, leash); `util/mob/MobSkillDescriptor`; released in `ExyliaLib.onPluginDisable` | [docs/mobs.md](docs/mobs.md) | 1.192.0 |
