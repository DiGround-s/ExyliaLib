...
| degradados sobre Components y mapas de caracteres | `text/Gradients` (`blend`, `at`, `wrap`, `paint`, `apply`, `length`), `text/CharMaps.transform` | `SmallText` es solo la tabla y delega en `CharMaps`; `Debug.gradientName` usa `Gradients.blend` | [docs/text.md](docs/text.md) | 1.102.0 |
...
| proxy (puente con ExyliaProxyUtils) | `proxy/Proxy`, `ProxyReply`; transporte por defecto de `command/PluginCommands.proxy()` | `proxy/internal/ProxyRuntime` (canal `exylia:bridge`, mapa de peticiones en vuelo, ping al primer join), `Wire`, `BridgeCommands`; `CrossServer.connect`/`connectOther` usan el módulo `connect` cuando el puente respondió y el canal `BungeeCord` si no | [docs/proxy.md](docs/proxy.md), [docs/teleport.md](docs/teleport.md) | 1.101.0, 1.102.0 |
...
| overlay | `overlay/Overlays`, `PluginOverlays`, `OverlayDefinition`, `OverlayLock`, `OverlaySlots`, `OverlayKeys` | `overlay/internal/` (`OverlayRuntime`, `OverlayView`, `OverlayClicks`, `OverlayLoader`, `OverlayListener`, `OverlaySink`; PacketEvents confinado en `OverlayPackets`) | [docs/overlays.md](docs/overlays.md) | 1.79.0 |
...
| util (loot) | `util/loot/Loot`, `LootEntry`, `LootType`, `LootCodec` (importar un cofre y editar propiedades, 1.77.0) | `util/loot/internal/` (`LootLines`, `LootRolls`, `LootItems`) | [docs/loot.md](docs/loot.md) | 1.56.0 |
