...
| metrics | — | `metrics/internal/` (`MetricsRuntime`, `ErrorGroups`, `ServerErrors`); `start` in `ExyliaLib.onEnable`, `stop` in `onDisable`; hooks in `AbstractTaskScheduler.report` and `Debug.send`; `metrics` block in `LibrarySettings` | [docs/metrics.md](docs/metrics.md) | 1.160.0 |
